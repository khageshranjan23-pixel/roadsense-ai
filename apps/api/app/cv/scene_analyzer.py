...
class SceneAnalyzer:
    ...
    def analyze_image(self, image_path: str) -> Dict[str, Any]:
        ...
        if self.has_local:
            ...
            # Gemini fallback for image
            if not self.api_key:
                raise ValueError("GEMINI_API_KEY is not set. Cannot run visual analysis in serverless mode.")
            ...
            prompt = """
            You are a road safety computer vision analyzer. Analyze this street view image.
            Detect all vehicles (cars, buses, trucks, bikes, auto-rickshaws) and pedestrians.
            Estimate:
            - track_id: unique integer for each object
            - class_name: one of "person", "car", "bus", "truck", "motorcycle", "bicycle", "traffic light"
            - bbox: bounding box coordinates as [x1, y1, x2, y2] relative to the image size (0 to width, 0 to height).
            - speed: speed in pixels per frame (0 to 20.0).
            - direction: one of "toward_crossing", "away_crossing", "lateral".
            - distance: estimated distance from the crossing line in pixels.
            
            Also identify:
            - traffic_light_color: "red", "green", "yellow", "none", or "unknown"
            - pedestrians_on_crossing: count of pedestrians on or near the crosswalk.
            - fastest_vehicle_speed: float speed.
            - closest_vehicle_distance: float distance.
            - scene_complexity: "simple", "moderate", or "complex" based on density.
              ...
              "objects": [
                {
                  ...
                  "bbox": [x1, y1, x2, y2],
                  "speed": 5.2,
                  "direction": "toward_crossing",
                  "distance": 120.0
                }
              ],
              "traffic_light_color": "red",
              "fastest_vehicle_speed": 5.2,
            ...
            try:
                ...
                if match:
                    ...
                    raise ValueError("Failed to parse Gemini response as JSON.") from e
                    
            # Draw bounding boxes using PIL
            draw = ImageDraw.Draw(pil_img)
            # Draw crossing line at 60% height
            line_y = int(0.6 * h)
            draw.line([(0, line_y), (w, line_y)], fill=(255, 165, 0), width=2)
            ...
            for obj in analysis.get("objects", []):
                ...
                if cname in {"car", "bus", "truck", "motorcycle", "bicycle"}:
                    color = (255, 0, 0) if speed > 8.0 else (255, 255, 0)
                    
                draw.rectangle(bbox, outline=color, width=2)
                
            # Draw traffic light status on top right
            draw.rectangle([(w - 220, 10), (w - 10, 50)], fill=(40, 40, 40))
            tl_color = analysis.get("traffic_light_color", "none")
            tl_rgb = (100, 100, 100)
            if tl_color == "red":
                tl_rgb = (255, 0, 0)
            elif tl_color == "green":
                ...
                tl_rgb = (255, 255, 0)
            draw.ellipse([(w - 210, 20), (w - 190, 40)], fill=tl_rgb)
            
            # Convert annotated PIL image back to base64
            buffered = io.BytesIO()
            pil_img.save(buffered, format="JPEG")
            base64_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
            
            analysis["annotated_frame_base64"] = base64_str
            analysis["recommended_freeze_frame"] = 0
    ...
    def analyze_video(self, video_path: str) -> Dict[str, Any]:
        ...
        if self.has_local:
            ...
            if video_file.state.name == "FAILED":
                raise ValueError(f"Video processing failed on Gemini API: {video_file.error.message}")
                
            model = genai.GenerativeModel("gemini-2.0-flash")
            
            prompt = """
            You are a road safety computer vision analyzer. Analyze this traffic video.
            Identify the situation at the most critical moments or when a vehicle is closest to the crosswalk.
            Detect all vehicles (cars, buses, trucks, bikes, auto-rickshaws) and pedestrians.
            Estimate for the critical freeze frame:
            - track_id: unique integer for each object
            - class_name: one of "person", "car", "bus", "truck", "motorcycle", "bicycle", "traffic light"
            - bbox: bounding box coordinates as [x1, y1, x2, y2] relative to the video frame size. Estimate coordinates assuming a standard 640x480 resolution (0 to 640 for x, 0 to 480 for y).
            - speed: speed in pixels per frame (0 to 20.0).
            - direction: one of "toward_crossing", "away_crossing", "lateral".
            - distance: estimated distance from the crossing line in pixels.
            ...
            - closest_vehicle_distance: float distance.
            - scene_complexity: "simple", "moderate", or "complex" based on density.
            - freeze_at_ms: timestamp in milliseconds of this critical moment in the video.
            ...
            # Generate a schematic base64 visual representation or placeholder for video freeze frame using PIL
            w, h = 640, 480
            pil_img = Image.new("RGB", (w, h), color=(5, 13, 26))
            draw = ImageDraw.Draw(pil_img)
            
            # Draw road lane
            draw.rectangle([(0, 200), (w, 360)], fill=(20, 28, 40))
            # Draw zebra crossing
            for i in range(0, w, 40):
                draw.rectangle([(i, 240), (i + 20, 320)], fill=(200, 200, 200))
            ...
            analysis["annotated_frame_base64"] = base64_str
            analysis["recommended_freeze_frame"] = 15
            
            return analysis

    def _draw_annotations(self, frame, objects: List[Dict[str, Any]], line_y: float, tl_color: str):
        """
        Draws boxes, labels, and safety markers on the image/frame using cv2.
        """
        h, w, _ = frame.shape
